...
\`\`\`bash
curl -fsSL https://raw.githubusercontent.com/yourusername/isp-management-system/main/install.sh | bash
\`\`\`

Or download and run locally:

\`\`\`bash
wget https://raw.githubusercontent.com/yourusername/isp-management-system/main/install.sh
chmod +x install.sh
./install.sh
\`\`\`
...
3. **Downloads** all required components from GitHub
...
# Reset database
docker compose down
docker volume rm isp-management-system_postgres_data
docker compose up -d
\`\`\`
