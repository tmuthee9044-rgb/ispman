...
import { Wifi, Globe, Zap, Clock, CheckCircle, Plus, Router, HardDrive } from 'lucide-react'
...
export function AddServiceModal({ open, onOpenChange, customerId, customerData }: AddServiceModalProps) {
  ...
  const [selectedCustomerRouter, setSelectedCustomerRouter] = useState("")
  ...
  // Mock customer routers from inventory
  const customerRouters = [
    {
      id: 1,
      brand: "TP-Link",
      model: "Archer C6",
      serial_number: "TL2024001234",
      status: "available",
    ...
    },
    {
      ...
      brand: "Huawei",
      model: "HG8245H",
      serial_number: "HW2024005678",
      status: "available",
      wifi_bands: "2.4GHz + 5GHz",
      max_speed: "1000Mbps",
      ports: "4 LAN + 1 WAN + 2 TEL",
      price: 12000.00,
    },
    {
      id: 3,
  ...
  const selectedCustomerRouterData = customerRouters.find((router) => router.id === Number.parseInt(selectedCustomerRouter))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("customer_router_id", selectedCustomerRouter)
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="text-center">
                        <div className="text-3xl font-bold">{formatKES(plan.price)}</div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center space-x-2">
                      ...
                      <span>Network Router Selection</span>
                    </CardTitle>
                    <CardDescription>Select the network router for this connection</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {routers.map((router) => (
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center space-x-2">
                      ...
                      <span>Customer Router Allocation</span>
                    </CardTitle>
                    <CardDescription>Allocate a router from inventory to the customer</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {customerRouters.map((router) => (
                        <Card
                          key={router.id}
                          className={`cursor-pointer transition-all ${
                            selectedCustomerRouter === router.id.toString()
                              ? "ring-2 ring-primary border-primary"
                              : "hover:shadow-sm"
                          } ${router.status === "reserved" ? "opacity-50" : ""}`}
                          onClick={() => router.status === "available" && setSelectedCustomerRouter(router.id.toString())}
                        >
                          <CardContent className="p-3">
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <div className="font-medium">{router.brand} {router.model}</div>
                                <div className="text-sm text-muted-foreground font-mono">S/N: {router.serial_number}</div>
                                <div className="text-sm text-muted-foreground">{router.wifi_bands} • {router.max_speed}</div>
                                <div className="text-sm text-muted-foreground">{router.ports}</div>
                                <div className="text-sm font-medium text-green-600">{formatKES(router.price)}</div>
                              </div>
                              <div className="text-right">
                                <Badge
                                  variant={router.status === "available" ? "default" : "secondary"}
                                  className={
                    ...
                    {selectedCustomerRouterData && (
                      <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                        <div className="text-sm font-medium text-blue-900">Selected Router Details</div>
                        <div className="text-sm text-blue-700 mt-1">
                          <div>Model: {selectedCustomerRouterData.brand} {selectedCustomerRouterData.model}</div>
                          <div>Serial: {selectedCustomerRouterData.serial_number}</div>
                          <div>Price: {formatKES(selectedCustomerRouterData.price)}</div>
                          <div>This router will be marked as "allocated" and assigned to the customer</div>
                        </div>
                      </div>
                    )}
                  </CardContent>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      ...
                      <div className="p-3 border rounded-lg">
                        ...
                        <div className="text-lg font-bold mt-2">{formatKES(selectedPlanData.price)}/month</div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        {selectedCustomerRouterData && (
                          <>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Customer Router:</span>
                              <span>{selectedCustomerRouterData.brand} {selectedCustomerRouterData.model}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Serial Number:</span>
                              <span className="font-mono">{selectedCustomerRouterData.serial_number}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Router Cost:</span>
                              <span>{formatKES(selectedCustomerRouterData.price)}</span>
                            </div>
                          </>
                        )}
                        <div className="flex justify-between">
