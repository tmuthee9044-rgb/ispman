    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">KES {currentMonth.revenue.toLocaleString()}</div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">KES {currentMonth.target.toLocaleString()} target</p>
          </CardContent>
        </Card>
              ...
              <div key={method.method} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  <div className="text-right">
                    <div className="font-semibold">KES {method.amount.toLocaleString()}</div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {analyticsData.customerSegments.map((segment) => (
              <div key={segment.segment} className="flex items-center justify-between p-4 border rounded-lg">
                ...
                <div className="text-right">
                  <div className="font-semibold">KES {segment.revenue.toLocaleString()}</div>
                  <div className="text-sm text-muted-foreground">
                    KES {Math.round(segment.revenue / segment.count).toLocaleString()} avg
                  </div>
                </div>
              </div>
            ))}
              ...
              return (
                <div key={month.month} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-4 text-sm">
                    <span className="font-semibold">KES {month.revenue.toLocaleString()}</span>
                    <span className="text-muted-foreground">/ KES {month.target.toLocaleString()}</span>
                    <Badge
